...
public class AppWidget extends AppWidgetProvider {
    ...
    public void onDeleted(Context context, int[] appWidgetsIds){
        ...
        //удаляем Preferences
        SharedPreferences.Editor editor = context.getSharedPreferences(ConfigurationActivity.WIDGET_PREF, Context.MODE_PRIVATE).edit();

        timer.cancel();
        for (int widgetsID : appWidgetsIds){
            editor.remove(ConfigurationActivity.WIDGET_DATE + widgetsID);
        }
    ...
    static void updateWidget(Context ctx, AppWidgetManager appWidgetManager, int widgetID){
        SharedPreferences sharedPreferences = ctx.getSharedPreferences(ConfigurationActivity.WIDGET_PREF, Context.MODE_PRIVATE);

        int widgetDay = sharedPreferences.getInt(ConfigurationActivity.WIDGET_DATE + widgetID, 0);
        int widgetMonth = sharedPreferences.getInt(ConfigurationActivity.WIDGET_MONTH + widgetID, 0);
        int widgetYear = sharedPreferences.getInt(ConfigurationActivity.WIDGET_YEAR + widgetID, 0);
        ...
        Intent configIntent = new Intent(ctx, ConfigurationActivity.class);
